...
const GenerarCuotaPorMultiplesPuestos: React.FC<Props> = ({ cuota }) => {
  ...
  const [puestosSeleccionados, setPuestosSeleccionados] = useState<string[]>([]);  // ✅ Array para múltiples puestos
  ...
  const handlePuestosChange = (event: SelectChangeEvent<string[]>) => {
    const value = event.target.value;
    setPuestosSeleccionados(typeof value === 'string' ? value.split(',') : value);
  };
  ...
  const registrarCuota = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Validar que se hayan seleccionado puestos
    if (puestosSeleccionados.length === 0) {
      mostrarAlerta("Advertencia", "Debe seleccionar al menos un puesto", "warning");
      return;
    }
    
    // Validar que se hayan agregado servicios
    if (serviciosIds.length === 0) {
      mostrarAlerta("Advertencia", "Debe agregar al menos un servicio", "warning");
      return;
    }
    ...
    try {
      ...
      if (cuota) {
        ...
        // Modo registro
        response = await apiClient.post(Api_Global_Cuotas.cuotas.registrarPorMultiplesPuestos(), dataToSend);
      }
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth required>
                  <InputLabel id="seleccionar-puesto-label">
                    Seleccionar Puestos (Múltiples)
                  </InputLabel>
                  <Select
                    multiple
                    labelId="seleccionar-puesto-label"
                    label="Seleccionar Puestos (Múltiples)"
                    id="select-puestos"
                    name="puestos"
...
export default GenerarCuotaPorMultiplesPuestos;
